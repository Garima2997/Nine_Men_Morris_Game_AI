...
def minmaxOpening(boardPosition, depth, resultantPosition,maximizingLevel):
    if(depth == 0):
    ...
    for pos in positions:
        if(maximizingLevel):
             whiteMinMax = minmaxOpening(pos, depth-1, resultantPosition,  False)
             if(whiteMinMax[0] > maxValue):
                 maxValue = whiteMinMax[0]
                 resultantPosition = pos
        else:
            blackMaxMin = minmaxOpening(pos, depth-1, resultantPosition, True)
...
if __name__ == "__main__":
    ...
    output = []
    resultantPosition = []
    output = minmaxOpening(inputBoardFile, depth, resultantPosition, True)
