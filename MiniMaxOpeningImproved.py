...
def minmaxOpeningImproved(boardPosition, depth, resultantPosition,maximizingLevel):
    if(depth == 0):
        MoveGenerator.positionEvaluated +=1
        return MoveGenerator.staticEstimationOpeningImproved(boardPosition), boardPosition
    ...
    for pos in positions:
        if(maximizingLevel):
             whiteMinMax = minmaxOpeningImproved(pos, depth-1, resultantPosition,  False)
             if(whiteMinMax[0] > maxValue):
                 maxValue = whiteMinMax[0]
                 resultantPosition = pos
        else:
            blackMaxMin = minmaxOpeningImproved(pos, depth-1, resultantPosition, True)
...
if __name__ == "__main__":
    ...
    output = []
    resultantPosition = []
    output = minmaxOpeningImproved(inputBoardFile, depth, resultantPosition, True)
